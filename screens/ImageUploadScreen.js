...
export default function ImageUploadScreen() {
  ...
  const pickImage = async () => {
    ...
    if (!result.cancelled) {
        if(result.type === "image") {
          ...
          Alert.alert("Please re-select image")
        }
    }
  };
  ...
  async function uploadImageAsync() {
    ...
    if(uploadResult?.type === "image"){
        const imageUri=  await getDownloadURL(fileRef);
        
        console.log("Download Uri is " , imageUri)
  ...
  return (
    <SafeAreaView style={styles.screen}>
      <View style={{ flex: 1, paddingTop: 50 , alignItems: "center"}}>
        ...
        {image && (
          <TouchableOpacity activeOpacity={0.5} onPress={uploadImageAsync}>
            <View
              style={{
                marginTop: 0,
